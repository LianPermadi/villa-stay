...
class VillaController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $villa = Villa::create($request->except('images', 'is_featured', 'amenities', 'primary_image_index') + [
    ...
    public function update(Request $request, Villa $villa)
    {
        $request->validate([
            ...
            'delete_images.*' => [
                'integer',
                Rule::exists('villa_images', 'id')->where('villa_id', $villa->id),
            ],
            'existing_primary_id' => [
                'nullable',
                'integer',
        ...
        $villa->update($request->except('images', 'is_featured', 'amenities', 'primary_image_index', 'existing_primary_id', 'delete_images') + [
            'is_featured' => $request->has('is_featured'),
            'amenities' => $amenities ? json_encode($amenities) : null,
        ]);

        $imagesToDelete = $villa->images()
        ...
        if (! $villa->images()->where('is_primary', true)->exists()) {
            $villa->images()->orderBy('sort_order')->orderBy('id')->first()?->update([
                'is_primary' => true,
            ]);
        }
